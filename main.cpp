...
void compressJPG(Mat image) {
    ...
    ImageBlockMatrix blockMatrix(image);

    // Step 1: Apply DCT and Quantization
    for (int i = 0; i < blockMatrix.getRows(); i++) {
        for (int j = 0; j < blockMatrix.getCols(); j++) {
            Mat block = blockMatrix.getBlockAt(i, j);
            Mat transformedBlock = applyDCT(block);
    ...
    // Step 2: For each block, encode and interleave Y, Cb, Cr
    for (int i = 0; i < blockMatrix.getRows(); i++) {
        for (int j = 0; j < blockMatrix.getCols(); j++) {
            Mat block = blockMatrix.getBlockAt(i, j);
            ...
            bitstream += encodeStandardDCDifference(diffY);
            prevDCY = encoded.dcY;
    ...
    // Optional: print bitstream in hex
    std::vector<uint8_t> packed = packBits(bitstream);
    std::cout << "Interleaved Bitstream: ";
    for (uint8_t byte : packed) printf("%02X ", byte);
    std::cout << "\nTotal Bitstream Length: " << bitstream.size() << " bits\n";
...
int main() {
    ...
    Mat img = imread(R"(..\test\images\cameraman.bmp)", IMREAD_GRAYSCALE);
    if (img.empty()) {
